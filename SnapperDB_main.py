...
def run_command(args):
    ...
    if args.command == 'fastq_to_db':
        ...
        # Hmmm, how does logging get handled here? all the qsubs will have their own logging processes
        #  but will be writing
        # to the same file
        ...
        logger.info(
            'PARAMS: config = %s; vcf = %s; AD ratio = %s; recombination_file = %s' % (args.config_file, args.vcf_file,
...
def main():
    ...
    parser_qsub_to_check_matrix = subparsers.add_parser('qsub_to_check_matrix',
                                                        help='This is only for internal use by snapperdb when '
                                                             'update matrix is being run in hpc mode.')
    parser_qsub_to_check_matrix.add_argument('-c', dest='config_file', metavar='Config file', required=True,
                                             help='The name of a config file in the user_configs directory '
                                                  '(not the full path)')
    parser_qsub_to_check_matrix.add_argument('-l', dest='strain_list', required=True,
                                             help='The list of all the strains in the SNPdb')
    parser_qsub_to_check_matrix.add_argument('-s', dest='short_strain_list', required=True,
                                             help='The list of all the strains already in the distance matrix')
    parser_qsub_to_check_matrix.add_argument('-u', dest='update_list', required=True,
                                             help='The list of all the strains to be added to the distance matrix')
    parser_qsub_to_check_matrix.add_argument('-g', dest='log_dir',
                                             default=os.path.join(os.path.expanduser('~'), 'logs'),
                                             help='Where do you want the logs written to? '
                                                  'Will default to a /user/home/logs')
    ...
    parser_update_clusters.add_argument('-g', dest='log_dir', default=os.path.join(os.path.expanduser('~'), 'logs'),
                                        help='Where do you want the logs written to? Will default to a /user/home/logs')
    parser_check_vcf_for_mixed = subparsers.add_parser('check_vcf_for_mixed',
                                                       help='Given the path to a vcf file, it will check the number '
                                                            'of positions in the vcf that have an AD ratio less than '
                                                            'specificed in -a')
    parser_check_vcf_for_mixed.add_argument('-c', dest='config_file', required=True,
                                            help='The name of a config file in the user_configs directory '
                                                 '(not the full path)')
    parser_check_vcf_for_mixed.add_argument('-v', dest='vcf_file', required=True, help='Path to a vcf')
    parser_check_vcf_for_mixed.add_argument('-a', dest='ad_ratio', required=True,
                                            help='Positions below this cutoff will be '
                                                 'counted by the script.')
    parser_check_vcf_for_mixed.add_argument('-o', dest='outdir', required=True,
                                            help='Where do you want the output to be '
                                                 'written to?')
    ...
    parser_get_variants_of_interest = subparsers.add_parser('get_variants_of_interest',
                                                            help='Given 2 strain lists, get_variants_of_interest will '
                                                                 'find high '
                                                                 'quality variants that differ between the two. '
                                                                 'One of the '
                                                                 'lists should be background, the other \'of interest\''
                                                                 '. '
                                                                 'Varaints present in the strains of interest will be '
                                                                 'printed. '
                                                                 'This function is in development and should be used '
                                                                 'with care.')
    parser_get_variants_of_interest.add_argument('-c', dest='config_file', required=True,
                                                 help='The name of a config file in the user_configs directory '
                                                      '(not the full path)')
    ...
    parser_get_variants_of_interest.add_argument('-g', dest='log_dir',
                                                 default=os.path.join(os.path.expanduser('~'), 'logs'),
                                                 help='Where do you want the logs written to? Will default to a '
                                                      '/user/home/logs')
    parser_upload_indels = subparsers.add_parser('upload_indels', help='Takes a indel vcf and uploads to a SNPdb with ')
    parser_upload_indels.add_argument('-c', dest='config_file', required=True,
                                      help='The name of a config file in the user_configs directory '
                                           '(not the full path)')
    parser_upload_indels.add_argument('-v', dest='vcf', nargs='+', required=True,
                                      help='Path to a vcf with indels called')
